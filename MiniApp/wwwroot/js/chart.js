...
function draw() {
    ...
    if (maxV > 0) {
        for (let i = 0; i < n; i++) {
            ...
            ctx.fillStyle = k.c >= k.o ? 'rgba(34,211,238,0.30)' : 'rgba(244,114,182,0.30)';
            ctx.fillRect(x, h - vh, bodyW, vh);
        }
    }

    // EMA-21 over closes
    const emas = emaSeries(candles.map(k => k.c), 21);
    ctx.strokeStyle = EMA_COLOR;
    ctx.lineWidth = 1.5;
    ctx.shadowColor = EMA_COLOR;
    ctx.shadowBlur = 6;
    ctx.beginPath();
    let started = false;
    for (let i = 0; i < n; i++) {
        if (!Number.isFinite(emas[i])) continue;
        const x = i * slot + slot / 2;
        if (!started) { ctx.moveTo(x, y(emas[i])); started = true; }
        else ctx.lineTo(x, y(emas[i]));
    }
    ctx.stroke();
    ctx.shadowBlur = 0;
    ...
    for (let i = 0; i < n; i++) {
        ...
        const dy = swayY(i);
        const glow = isForming ? 6 + 10 * pulse : 5;
