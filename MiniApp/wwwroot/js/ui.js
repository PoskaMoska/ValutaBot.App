...
function renderAiChartLoop() {
    ...
    if (aiChartData.length === 0) {
        ...
        aiChartData.forEach((c, i) => {
            let cx = padding + i * spacing + spacing/2;
            let cy = padding + (maxP - c.close) * scaleY;
        ...
        aiChartData.forEach((c, i) => {
            ...
            const x = padding + i * spacing + (spacing - candleWidth)/2 + swayX;
            const yHigh = padding + (maxP - c.high) * scaleY + swayY;
            const yLow = padding + (maxP - c.low) * scaleY + swayY;
            const yOpen = padding + (maxP - Math.max(c.open, c.close)) * scaleY + swayY;
            let bodyH = Math.abs(c.close - c.open) * scaleY;
            if(bodyH < 1) bodyH = 1;
            
            const color = isBull ? '#0ea5e9' : '#d946ef';
            const glow = isBull ? 'rgba(14,165,233,0.5)' : 'rgba(217,70,239,0.5)';
