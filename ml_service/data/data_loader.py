...
def _fetch_rl_feedback(symbol: str, interval: str) -> List[Dict]:
    ...
    if db_url:
        try:
            ...
            # Fix #9: Only use trades AFTER 2026-09-26 (feature-name fix deployed).
            # Pre-fix trades had 37.8% winrate based on random ML signals (model was
            # predicting on a zero-filled row due to Column_N mismatch). Injecting those
            # corrupted outcomes as RL labels would invert valid LightGBM signal.
            # New trades (post-fix) are generated by a correctly-aligned model and are safe.
            RL_FEEDBACK_CUTOFF = os.getenv("RL_FEEDBACK_CUTOFF", "2026-09-26")
            query = """
                SELECT
                    created_at         AS ts,
                    direction          AS dir,
            ...
            """
            df = pd.read_sql_query(query, conn, params=(symbol, interval, RL_FEEDBACK_CUTOFF))
            conn.close()
